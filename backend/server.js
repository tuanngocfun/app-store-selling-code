...
app.post('/admin/signin', async (req, res) => {
  try {
    ...
    const admin = await pool.query('SELECT * FROM rsn_admin WHERE email = $1 AND EXISTS (SELECT 1 FROM rsn_admin WHERE email = $1)', [
      email,
    ]);
    ...
    if (!registeredAdmin) {
      ...
      if (await bcrypt.compare(password, registeredAdmin.password)) {
        ...
        const token = jwt.sign({ email: registeredAdmin.email, password: registeredAdmin.password }, JWT_SECRET_KEY);
    ...
    if (authHeader) {
      ...
      jwt.verify(token, JWT_SECRET_KEY, (err, userDetails) => {
        ...
        const getUser = async () => {
          ...
          const responseData = [userDetails, wishNumb.rows[0], wishDetails.rows, library];
...
app.put('/admin/:id', async (req, res) => {
  try {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      const deletePaths = [deletePath1, deletePath2, deletePath3, deletePath4, deletePath5, deletePath6, deletePath7];
...
app.post('/user/signin', async (req, res) => {
  try {
    ...
    const user = await pool.query('SELECT * FROM rsn_user WHERE email = $1 AND EXISTS (SELECT 1 FROM rsn_user WHERE email = $1)', [email]);
    ...
    if (!registeredUser) {
      ...
      if (await bcrypt.compare(password, registeredUser.password)) {
        // console.log(req.body)
        const token = jwt.sign({ email: registeredUser.email, password: registeredUser.password }, JWT_SECRET_KEY);
  ...
  try {
    ...
    if (authHeader) {
      ...
      jwt.verify(token, JWT_SECRET_KEY, async (err, userDetails) => {
        ...
        const validUser = jwt.decode(token);
        const productid = req.body.productid;
        const query = await pool.query('SELECT userid FROM rsn_user WHERE email = $1', [validUser.email]).then(async query => {
          const userID = query.rows[0].userid;
          const getOrders = await pool
            .query('SELECT orderid, totalprice, ordered_at FROM rsn_order WHERE userid = $1 ORDER BY ordered_at DESC', [userID])
    ...
    if (authHeader) {
      ...
      jwt.verify(token, JWT_SECRET_KEY, async (err, userDetails) => {
        ...
        const query = await pool
          .query('SELECT orderid, totalprice, ordered_at FROM rsn_order WHERE orderid = $1', [orderid])
          .then(query => res.json(query.rows));
      });
    }
  } catch (error) {console.log(error.message);}
  ...
  try {
    ...
    if (authHeader) {
      ...
      jwt.verify(token, JWT_SECRET_KEY, async (err, userDetails) => {
        ...
        const query = await pool.query('SELECT userid FROM rsn_user WHERE email = $1', [validUser.email]);
        const userID = query.rows[0].userid;
        const queryWish = await pool.query('INSERT INTO rsn_wishlist(productid, userid, status) VALUES ($1, $2 , true)', [
          productid,
          userID,
        ]);
  ...
  try {
    const removeProduct = async () => {
      ...
      const deleteQuery = await pool
        .query('DELETE FROM rsn_inventory WHERE inventid = $1 AND cartid = $2', [inventid, cartid])
        .then(async () => {
          const updateQuery = await pool.query('UPDATE rsn_cart SET totalcart = totalcart - $1,  quantity = $2 WHERE cartid = $3', [
            price,
            quantity,
            cartid,
          ]);
            ...
            if (result === false) {
              ...
              const transaction = async () => {
                const amountQuery = await pool
                  .query('UPDATE rsn_credit SET amount = amount - $1', [price])
                  .then(async () => {
                    const userQuery = await pool
                      .query('SELECT userid FROM rsn_user WHERE email = $1', [validUser.email])
                      .then(async userQuery => {
                        const userid = userQuery.rows[0].userid;
                        const insertOrder = await pool
                          .query('INSERT INTO rsn_order(userid, totalprice, quantity) VALUES ($1, $2, $3)', [userid, price, quantity])
                          .then(async () => {
                            ...
                            for (let i = 0; i < quantity; i++) {
                              ...
                              const insertItem = async () => {
                                const insertOrderItem = await pool.query(
                                  'INSERT INTO rsn_order_items (productid, orderid, code) VALUES ($1, $2, $3)',
                                  [items[i].productid, orderID, code]
                                );
                              };
                              insertItem();
                            }
                          ...
                          .then(async () => {
                            const getCart = await pool
                              .query('SELECT cartid FROM rsn_cart NATURAL JOIN rsn_user WHERE userid = $1', [userid])
                              .then(async getCartQuery => {
                                const cartID = getCartQuery.rows[0].cartid;
                                const deleteInventoryQuery = await pool
                                  .query('DELETE FROM rsn_inventory WHERE cartid = $1', [cartID])
                                  .then(async () => {
